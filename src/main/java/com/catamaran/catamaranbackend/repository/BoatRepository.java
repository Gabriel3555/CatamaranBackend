...
public interface BoatRepository extends JpaRepository<BoatEntity, Long> {
    ...
    // Métodos para búsqueda y filtrado
      @Query("SELECT b FROM BoatEntity b WHERE " +
                 ...
                 "(:status = 'all' OR (:status = 'Disponible' AND b.owner IS NULL) OR (:status = 'Ocupado' AND b.owner IS NOT NULL))")
      Page<BoatEntity> findWithFilters(@Param("search") String search,
                                         @Param("type") com.catamaran.catamaranbackend.domain.BoatType type,
    ...
    // Método para filtrar por tipo
      ...
      Page<BoatEntity> findByType(@Param("type") com.catamaran.catamaranbackend.domain.BoatType type,
                                    Pageable pageable);
